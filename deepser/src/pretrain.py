...
class DeepSERBase(nn.Module):
    def __init__(self, embed_dim1, mlp_out1, embed_dim2, mlp_out2, embed_dim3, mlp_out3, mlp_out4, encoder = "paper", dropout=0.1, mixup=0.0, pretrained_encoder_path="pretrained"):
        """
        ...
        # Load pre-trained weights for leg1 if provided
        if pretrained_encoder_path is not None:
            self.load_pretrained_encoders(pretrained_encoder_path)
    ...
    def load_pretrained_encoders(self, pretrained_path):
        """Load pre-trained encoder weights for leg1"""
        try:
            leg1_weights = torch.load(pretrained_path+"/leg1.pt", map_location='cpu')
            self.leg1.load_state_dict(leg1_weights)
            leg2_weights = torch.load(pretrained_path+"/leg2.pt", map_location='cpu')
            self.leg2.load_state_dict(leg2_weights)
            leg3_weights = torch.load(pretrained_path+"/leg3.pt", map_location='cpu')
            self.leg3.load_state_dict(leg3_weights)
            print(f"Successfully loaded pre-trained encoder weights from {pretrained_path}")
        except Exception as e:
            print(f"Warning: Could not load pre-trained encoder weights from {pretrained_path}: {e}")
...
class SimpleEncoderModel(nn.Module):
    """
    Simple model consisting of just the BaseEnc encoder and regression heads
    to match DeepSERBase's output structure
    """
    def __init__(self, embed_dim, mlp_out, dropout=0.1, mixup=0.0):
        super(SimpleEncoderModel, self).__init__()
        print(f"Using SimpleEncoderModel: \n\tembed_dim={embed_dim}\n\tmlp_out={mlp_out}\n\tdropout={dropout}\n\tmixup={mixup}")
        
        self.mixup = mixup
        # Only leg1 (language/text encoder)
        self.encoder = BaseEnc(embed_dim, mlp_out, dropout)
        
        # Regression heads to match DeepSERBase output
        self.linear1 = nn.Linear(mlp_out, mlp_out)
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(mlp_out, 8)  # Classification head
        self.linear3 = nn.Linear(mlp_out, 3)  # Regression head
        
    def forward(self, x1=None, x2=None, x3=None, labels=None, dev=False):
        """
        Forward pass - only uses x1 (language/text), ignores x2 and x3
        """
        # Only process the first modality (language/text)
        x, f, s = self.encoder(x3)
        ...
        # Get outputs
        class_out = self.linear2(x)
        reg_out = self.linear3(x)
        reg_out = 1 + 6*torch.sigmoid(reg_out)
...
class MULTModel(nn.Module):
   ...
   def __init__(self, hyp_params):
      ...
      #  self.model = SimpleEncoderModel(
      #       embed_dim=self.orig_d_v,  # modality dimension
      #       mlp_out=hidden_dim,
      #       dropout=dropout,
      #       mixup=mixup
      #   )
   ...
   def save_encoder_weights(self, path):
        ...
        self.model.save_encoder_weights(path)
